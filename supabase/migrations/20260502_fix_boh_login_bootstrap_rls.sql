-- Restore authenticated BOH login/bootstrap reads under PostgREST/RLS.
--
-- The frontend resolves the Supabase auth user to public.boh_user, then reads
-- role/app assignments from public.boh_user_role and public.boh_user_app.
-- Without grants and select policies on those bootstrap tables, production/dev
-- login lands on /boh but the REST calls fail with 403 Forbidden.
...
-- Login may link an invited BOH row to the freshly authenticated Supabase user
-- when the BOH row has the same email but no auth_user_id yet.
