...
-- The operational Vault is central. Bootstrap existing active Vault administrators
-- into Production so they can create the first Production-scoped items and grants.
...
select
  gen_random_uuid(),
  development_admin.tenant_id,
  development_admin.boh_user_id,
  development_admin.role,
  'production',
  'active',
  development_admin.expires_at,
  development_admin.granted_by
from public.boh_vault_access_grants development_admin
where development_admin.environment = 'development'
  and development_admin.role = 'vault_admin'
  and development_admin.status = 'active'
  and (development_admin.expires_at is null or development_admin.expires_at > now())
  and not exists (
    select 1
    from public.boh_vault_access_grants production_admin
    where production_admin.tenant_id = development_admin.tenant_id
      and production_admin.boh_user_id = development_admin.boh_user_id
      and production_admin.role = 'vault_admin'
      and production_admin.environment = 'production'
      and production_admin.status = 'active'
  );

-- Claim one exact queued synchronization run from either canonical Vault
-- environment. Exact item, field, destination, connection, version, role, and
-- adapter checks remain enforced by the existing synchronization contracts.
...
begin
  ...
  if run_environment not in ('development', 'production') then
...
comment on function public.boh_vault_claim_sync_run(uuid,uuid,uuid,text,text) is
  'Service claim for one exact Development or Production Vault synchronization run. Returns encrypted envelope and approved destination metadata, never plaintext.';
