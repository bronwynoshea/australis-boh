...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!identity.firstName || !identity.lastName) return json(req, { error: "boh_user_onboarding_incomplete", message: "First name and last name are required before creating a Personal Table." }, 400);

    const { data: tenant, error: tenantError } = await supabaseAdmin.from("boh_tenant").select("id, slug").eq("id", identity.tenantId).single();
    ...
    async function returnExistingPersonalRoom(existingRoom: any) {
      ...
      await supabaseAdmin.from("loft_room_member").upsert({ loft_room_id: existingRoom.id, boh_user_id: identity.bohUserId, role: "host" }, { onConflict: "loft_room_id,boh_user_id" });
    ...
    const { data: room, error: insertError } = await supabaseAdmin.from("loft_room").insert({
      app_context: 'cafe',
      tenant_id: tenantId,
