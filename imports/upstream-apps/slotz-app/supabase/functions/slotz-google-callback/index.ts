...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (error) {
      console.error('Google OAuth returned error:', error, errorDescription)
      const appUrl = await getAppUrlForCallback(req)
      return redirect(`${appUrl}/?slotz_google=error`)
    }
    ...
    if (!staffId || !createdAt || !codeVerifier || Date.now() - createdAt > 10 * 60 * 1000) {
    ...
    return redirect(`${appUrl}/?slotz_google=connected`)
  } catch (error) {
    console.error('Google callback error:', error)
    const reason = classifyCallbackError(error)
    await recordCallbackFailure(req, reason)
    const appUrl = await getAppUrlForCallback(req)
    return redirect(`${appUrl}/?slotz_google=error&reason=${reason}`)
  }
})
...
async function saveGoogleConnection(supabase: any, staffId: string, code: string, codeVerifier: string) {
  ...
  await supabase.from('google_oauth_tokens').update({ is_active: false }).eq('staff_id', staffId)
  ...
  const { error: tokenError } = await supabase
    ...
    .insert({
      ...
      account_id: profile.id || profile.sub || profile.email,
      account_username: profile.email,
      account_name: profile.name,
      token_type: tokenData.token_type || 'Bearer',
      scope: tokenData.scope || 'openid email profile https://www.googleapis.com/auth/calendar.events',
      is_active: true,
...
async function exchangeCode(supabase: any, code: string, codeVerifier: string) {
  const params = new URLSearchParams({
    ...
    client_secret: await getGoogleClientSecret(supabase),
    code,
    code_verifier: codeVerifier,
    redirect_uri: requiredEnv('SLOTZ_GOOGLE_REDIRECT_URI'),
...
async function getGoogleClientSecret(supabase: any) {
  const envSecret = Deno.env.get('SLOTZ_GOOGLE_CLIENT_SECRET')
  if (envSecret) return envSecret

  const { data, error } = await supabase
    .from('vault.decrypted_secrets')
    .select('decrypted_secret')
    .eq('name', 'SLOTZ_GOOGLE_CLIENT_SECRET')
    .single()

  if (error || !data?.decrypted_secret) throw new Error('Missing SLOTZ_GOOGLE_CLIENT_SECRET')
...
async function fetchGoogleProfile(accessToken: string) {
  ...
  const body = await response.json()
  if (!response.ok) throw new Error(body.error?.message || 'Google profile lookup failed')
  return body
}
...
function classifyCallbackError(error: unknown) {
  ...
  if (message.includes('token') || message.includes('client_secret') || message.includes('authorization code')) return 'token_exchange'
  if (message.includes('refresh token')) return 'missing_refresh_token'
  if (message.includes('profile') || message.includes('userinfo')) return 'profile'
  if (message.includes('google_oauth_tokens')) return 'token_save'
  if (message.includes('google_calendar_sync')) return 'sync_save'
  ...
  try {
    ...
    await supabase
      ...
      .upsert({
        ...
        last_sync_error: `google_callback_${reason}`,
        updated_at: new Date().toISOString(),
      }, { onConflict: 'staff_id' })
  } catch (diagnosticError) {
    console.error('Could not record Google callback failure:', diagnosticError)
  }
}
