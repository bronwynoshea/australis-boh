...
serve(async (req: Request) => {
  ...
  try {
                ...
                <tr>
                  <td style="padding: 48px;">
                    ...
                    <div style="color: #1a1a1a; line-height: 1.6; font-size: 16px;">
                      <h2 style="font-size: 24px; font-weight: 900; margin-bottom: 20px; color: #111;">New Booking</h2>
                      <p>A new ${meetingType.name} has been added to the JOBZ CAFE® SLOTZ Calendar.</p>
                      <div style="background: #f4f4f7; padding: 32px; border-radius: 24px; margin: 32px 0;">
                        ...
                        ${booking.guest_notes ? `<p style="margin: 0; font-size: 14px; color: #666; border-left: 4px solid #635CCD; padding-left: 16px;"><em>Guest Notes: ${booking.guest_notes}</em></p>` : ''}
                      </div>
    ...
    if (!resendApiKey) {
      console.error('SLOTZ_RESEND_API_KEY environment variable not set')
      return new Response(
        JSON.stringify({ error: 'SLOTZ_RESEND_API_KEY not configured' }),
        { status: 500 }
      )
    }
