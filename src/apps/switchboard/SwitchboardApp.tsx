...
const selectInput = `${input} appearance-none pr-8`;
...
function LinkResourceDialog({ project, vaultItems, onClose, onLinked }: { project: SwitchboardProject; vaultItems: Array<{ id: string; display_name: string; environment: string }>; onClose: () => void; onLinked: () => Promise<void> }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      toast.success('Service resource linked');
      await onLinked();
      onClose();
    } catch (error) {
      toast.error(error instanceof Error ? error.message : 'Unable to link service resource.');
    } finally { setSaving(false); }
  };
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/45 p-4" role="dialog" aria-modal="true" aria-labelledby="link-switchboard-resource-title"><div className={`${surface} max-h-[92vh] w-full max-w-2xl overflow-y-auto p-6`}><div className="flex items-start justify-between gap-4"><div><h2 id="link-switchboard-resource-title" className="text-lg font-semibold">Link service resource</h2><p className="mt-1 text-sm text-boh-text-sub-light dark:text-boh-text-sub">Attach a provider account and resource to {project.name}.</p></div><button className={secondaryButton} onClick={onClose} aria-label="Close"><X size={16}/></button></div><div className="mt-5 grid gap-4 sm:grid-cols-2"><label className="block text-sm font-medium">Provider<select className={`${selectInput} mt-1.5`} value={provider} onChange={(event)=>chooseProvider(event.target.value as SwitchboardProvider)}>{Object.keys(providerKinds).map((value)=><option key={value} value={value}>{formatSwitchboardStatus(value)}</option>)}</select></label><label className="block text-sm font-medium">Environment scope<select className={`${selectInput} mt-1.5`} value={scope} onChange={(event)=>{const next=event.target.value as SwitchboardEnvironmentScope;setScope(next);if(next==='shared')setCredentialId('');}}><option value="shared">Shared</option><option value="development">Development</option><option value="production">Production</option></select></label><label className="block text-sm font-medium">Connection name<input className={`${input} mt-1.5`} value={connectionName} onChange={(event)=>setConnectionName(event.target.value)} /></label><label className="block text-sm font-medium">Connection key<input className={`${input} mt-1.5 font-mono`} value={connectionKey} onChange={(event)=>setConnectionKey(normalizeSwitchboardKey(event.target.value))} /></label><label className="block text-sm font-medium">Account or organization <span className="font-normal text-boh-text-sub-light dark:text-boh-text-sub">Optional</span><input className={`${input} mt-1.5`} value={accountName} onChange={(event)=>setAccountName(event.target.value)} /></label><label className="block text-sm font-medium">Account ID <span className="font-normal text-boh-text-sub-light dark:text-boh-text-sub">Optional</span><input className={`${input} mt-1.5 font-mono`} value={accountId} onChange={(event)=>setAccountId(event.target.value)} /></label><label className="block text-sm font-medium sm:col-span-2">Vault credential <span className="font-normal text-boh-text-sub-light dark:text-boh-text-sub">Optional</span><select disabled={scope==='shared'} className={`${selectInput} mt-1.5 disabled:cursor-not-allowed disabled:opacity-60`} value={credentialId} onChange={(event)=>setCredentialId(event.target.value)}><option value="">{scope==='shared'?'Not available for shared resources':'Needs setup'}</option>{vaultItems.filter((item)=>item.environment===scope).map((item)=><option key={item.id} value={item.id}>{item.display_name} · {formatSwitchboardStatus(item.environment)}</option>)}</select></label><label className="block text-sm font-medium">Resource type<select className={`${selectInput} mt-1.5`} value={resourceKind} onChange={(event)=>setResourceKind(event.target.value as SwitchboardResourceKind)}>{providerKinds[provider].map((value)=><option key={value} value={value}>{formatSwitchboardStatus(value)}</option>)}</select></label><label className="block text-sm font-medium">Resource name<input className={`${input} mt-1.5`} value={resourceName} onChange={(event)=>setResourceName(event.target.value)} placeholder="JOBZCAFE® repository" /></label><label className="block text-sm font-medium sm:col-span-2">Provider resource ID<input className={`${input} mt-1.5 font-mono`} value={externalId} onChange={(event)=>setExternalId(event.target.value)} placeholder="Repository, project, worker, or domain identifier" /></label><label className="block text-sm font-medium sm:col-span-2">Service URL <span className="font-normal text-boh-text-sub-light dark:text-boh-text-sub">Optional</span><input type="url" className={`${input} mt-1.5`} value={serviceUrl} onChange={(event)=>setServiceUrl(event.target.value)} placeholder="https://…" /></label></div><div className="mt-6 flex justify-end gap-2"><button className={secondaryButton} onClick={onClose}>Cancel</button><button className={primaryButton} disabled={saving||!connectionName.trim()||!connectionKey||!resourceName.trim()||!externalId.trim()} onClick={()=>void save()}>{saving?'Linking…':'Link resource'}</button></div></div></div>;
}
...
function EditResourceDialog({ resource, onClose, onSaved }: { resource: SwitchboardResource; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      toast.success('Service resource updated');
      await onSaved();
      onClose();
    } catch (error) {
      toast.error(error instanceof Error ? error.message : 'Unable to update service resource.');
    } finally { setSaving(false); }
  };
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/45 p-4" role="dialog" aria-modal="true" aria-labelledby="edit-switchboard-resource-title"><div className={`${surface} w-full max-w-lg p-6`}><div className="flex items-start justify-between gap-4"><div><h2 id="edit-switchboard-resource-title" className="text-lg font-semibold">Edit service resource</h2><p className="mt-1 text-sm text-boh-text-sub-light dark:text-boh-text-sub">Update the visible name, provider resource, and app or service URL.</p></div><button className={secondaryButton} onClick={onClose} aria-label="Close"><X size={16}/></button></div><div className="mt-5 space-y-4"><label className="block text-sm font-medium">Resource name<input autoFocus className={`${input} mt-1.5`} value={displayName} onChange={(event)=>setDisplayName(event.target.value)} /></label><label className="block text-sm font-medium">Provider resource name<input className={`${input} mt-1.5`} value={externalResourceName} onChange={(event)=>setExternalResourceName(event.target.value)} /></label><label className="block text-sm font-medium">Provider resource ID<input className={`${input} mt-1.5 font-mono`} value={externalResourceId} onChange={(event)=>setExternalResourceId(event.target.value)} /></label><label className="block text-sm font-medium">App or service URL <span className="font-normal text-boh-text-sub-light dark:text-boh-text-sub">Optional</span><input type="url" className={`${input} mt-1.5`} value={serviceUrl} onChange={(event)=>setServiceUrl(event.target.value)} placeholder="https://…" /></label></div><div className="mt-6 flex justify-end gap-2"><button className={secondaryButton} onClick={onClose}>Cancel</button><button className={primaryButton} disabled={saving||!displayName.trim()||!externalResourceName.trim()||!externalResourceId.trim()} onClick={()=>void save()}>{saving?'Saving…':'Save changes'}</button></div></div></div>;
}
...
function OverviewPage({ snapshot }: { snapshot: SwitchboardSnapshot }) {
  ...
  const cards = [
    ...
    { label: 'Service resources', value: snapshot.resources.filter((resource)=>resource.status==='active').length, icon: Link2 },
...
function ProjectsPage({ snapshot, vaultItems, refresh, canEdit }: { snapshot: SwitchboardSnapshot; vaultItems: Array<{ id: string; display_name: string; environment: string }>; refresh: () => Promise<void>; canEdit: boolean }) {
...
function ResourceRow({ resource, connections, canEdit, onEdit }: { resource: SwitchboardResource; connections: SwitchboardConnection[]; canEdit?: boolean; onEdit?: () => void }) {
  ...
  return <div className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><div className="flex items-center gap-2"><span className="font-medium">{resource.display_name}</span><span className="text-xs text-boh-text-sub-light dark:text-boh-text-sub">{formatSwitchboardStatus(resource.environment_scope)}</span></div><p className="mt-1 break-all font-mono text-xs text-boh-text-sub-light dark:text-boh-text-sub">{resource.external_resource_id}</p>{resource.service_url&&<p className="mt-1 break-all text-xs text-boh-text-sub-light dark:text-boh-text-sub">{resource.service_url}</p>}</div><div className="flex items-center gap-3 text-sm"><span>{formatSwitchboardStatus(provider)}</span>{canEdit&&<button className={secondaryButton} onClick={onEdit}>Edit</button>}</div></div>;
}

function ServicesPage({ snapshot }: { snapshot: SwitchboardSnapshot }) {
  return <><PageHeader eyebrow="Provider accounts" title="Services" description="Review connected accounts and the project resources authorized through each service."/>{snapshot.connections.length===0?<EmptyState title="No services linked" description="Link a service resource from a project to create its account connection."/>:<div className="space-y-4">{snapshot.connections.map((connection)=>{const resources=snapshot.resources.filter((resource)=>resource.connection_id===connection.id);return <section key={connection.id} className={`${surface} overflow-hidden`}><div className="flex flex-col gap-3 border-b border-boh-border-light p-5 dark:border-boh-border sm:flex-row sm:items-start sm:justify-between"><div><p className="text-xs font-semibold uppercase tracking-wide text-emerald-700 dark:text-emerald-300">{formatSwitchboardStatus(connection.provider)}</p><h2 className="mt-1 text-lg font-semibold">{connection.display_name}</h2><p className="mt-1 text-sm text-boh-text-sub-light dark:text-boh-text-sub">{connection.external_account_name||connection.external_account_id||'Account details not recorded'}</p></div><div className="text-sm"><p className="font-medium">{formatSwitchboardStatus(connection.status)}</p><p className="mt-1 text-xs text-boh-text-sub-light dark:text-boh-text-sub">{connection.credential_vault_item_id?'Vault credential linked':'Vault credential needs setup'}</p></div></div><div className="divide-y divide-boh-border-light dark:divide-boh-border">{resources.map((resource)=><React.Fragment key={resource.id}><ResourceRow resource={resource} connections={snapshot.connections}/></React.Fragment>)}</div></section>})}</div>}</>;
}
...
const SwitchboardApp: React.FC<{ isAdmin?: boolean }> = ({ isAdmin=false }) => {
  ...
  const [vaultItems,setVaultItems]=useState<Array<{id:string;display_name:string;environment:string}>>([]);
