...
const DeviceDropdown: React.FC<DeviceDropdownProps> = ({ label, value, onChange, options }) => {
  ...
  }, [hasOptions, options, value, onChange]);
  ...
  return (
    <div ref={rootRef} className="space-y-2 relative overflow-visible">
      ...
      <button
        ...
        disabled={!hasOptions}
        ref={triggerRef}
        className="w-full loft-panel rounded-xl px-4 py-3 text-sm font-medium text-main/70 dark:text-white/70 outline-none flex items-center justify-between gap-3 disabled:cursor-not-allowed disabled:opacity-60"
      >
        <span className="truncate text-left flex-1">{selectedLabel || 'No device available'}</span>
