...
function ServicesPage({ snapshot }: { snapshot: SwitchboardSnapshot }) {
  return <><PageHeader eyebrow="Provider accounts" title="Services" description="Review connected accounts and the project services authorized through each provider."/>{snapshot.connections.length===0?<EmptyState title="No services linked" description="Link a service from a project to create its account connection."/>:<div className="space-y-4">{snapshot.connections.map((connection)=>{const resources=snapshot.resources.filter((resource)=>resource.connection_id===connection.id);const credentialText=connection.credential_vault_item_id?'Provider credential linked':'Provider credential not linked';return <section key={connection.id} className={`${surface} overflow-hidden`}><div className="flex flex-col gap-3 border-b border-boh-border-light p-5 dark:border-boh-border sm:flex-row sm:items-start sm:justify-between"><div><p className="text-xs font-semibold uppercase tracking-wide text-emerald-700 dark:text-emerald-300">{formatSwitchboardStatus(connection.provider)}</p><h2 className="mt-1 text-lg font-semibold">{connection.display_name}</h2><p className="mt-1 text-sm text-boh-text-sub-light dark:text-boh-text-sub">{connection.external_account_name||connection.external_account_id||'Account details not recorded'}</p></div><div className="text-sm"><p className="font-medium">{credentialText}</p><p className="mt-1 max-w-xs text-xs text-boh-text-sub-light dark:text-boh-text-sub">This is the provider account credential status. It does not mean every linked project service is broken.</p></div></div><div className="divide-y divide-boh-border-light dark:divide-boh-border">{resources.map((resource)=><React.Fragment key={resource.id}><ResourceRow resource={resource} connections={snapshot.connections}/></React.Fragment>)}</div></section>})}</div>}</>;
}
