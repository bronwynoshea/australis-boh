-- CELLAR staff review actions for investor access requests.
...
begin
  if p_investor_profile_id is null then
    raise exception 'CELLAR_INVESTOR_PROFILE_REQUIRED' using errcode = '22023';
  end if;

  if p_boh_user_id is null or trim(p_boh_user_id) = '' then
    raise exception 'CELLAR_BOH_USER_REQUIRED' using errcode = '22023';
  end if;

  if cellar_action not in ('approve', 'need_more_info', 'decline') then
    raise exception 'CELLAR_REVIEW_ACTION_INVALID' using errcode = '22023';
  end if;

  select *
    into cellar_profile
  from public.cellar_investor_profiles cip
  where cip.id = p_investor_profile_id
  for update;

  if cellar_profile.id is null then
    raise exception 'CELLAR_INVESTOR_PROFILE_NOT_FOUND' using errcode = 'P0002';
  end if;

  if cellar_action = 'approve' then
    cellar_access_status := 'verified';
    cellar_profile_status := 'verified';
    cellar_pipeline_status := 'new_investor';
  elsif cellar_action = 'need_more_info' then
    cellar_access_status := 'verification_pending';
    cellar_profile_status := 'needs_more_info';
    cellar_pipeline_status := 'needs_more_info';
  else
    cellar_access_status := 'revoked';
    cellar_profile_status := 'rejected';
    cellar_pipeline_status := 'declined';
  end if;
  ...
  set
    ...
    assigned_boh_user_id = case when cellar_action = 'approve' then p_boh_user_id else cia.assigned_boh_user_id end,
    ...
    metadata = cia.metadata
      || coalesce(p_metadata, '{}'::jsonb)
      || jsonb_build_object(
        'last_cellar_review_action', cellar_action,
        'last_cellar_reviewed_at', now(),
        'last_cellar_reviewed_by_boh_user_id', p_boh_user_id
      )
