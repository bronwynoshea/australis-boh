...
function ServicesPage({ snapshot }: { snapshot: SwitchboardSnapshot }) {
  ...
  return <><PageHeader eyebrow="Provider inventory" title="Services" description="Scan services by provider. Use Projects to manage a product's service links and Vault items together."/>{providers.length===0?<EmptyState title="No services linked" description="Link a service from a project to create its provider inventory."/>:<div className="space-y-4">{providers.map(({ provider, connections, resources })=>{const linkedCredentials=connections.filter((connection)=>connection.credential_vault_item_id).length;return <section key={provider} className={`${surface} overflow-hidden`}><div className="flex flex-wrap items-center justify-between gap-3 border-b border-boh-border-light px-4 py-3 dark:border-boh-border"><div><p className="text-xs font-semibold uppercase tracking-wide text-emerald-700 dark:text-emerald-300">{formatSwitchboardStatus(provider)}</p><h2 className="text-base font-semibold">{resources.length} services · {connections.length} connections</h2></div><p className="text-xs text-boh-text-sub-light dark:text-boh-text-sub">{linkedCredentials}/{connections.length} provider credentials linked</p></div>{resources.length===0?<p className="px-4 py-3 text-sm text-boh-text-sub-light dark:text-boh-text-sub">No project services linked for this provider.</p>:<div className="overflow-x-auto"><table className="w-full min-w-[720px] text-left text-sm"><thead className="bg-boh-bg-light text-xs uppercase tracking-wide text-boh-text-sub-light dark:bg-boh-bg dark:text-boh-text-sub"><tr><th className="px-4 py-2 font-medium">Project</th><th className="px-4 py-2 font-medium">Service</th><th className="px-4 py-2 font-medium">Environment</th><th className="px-4 py-2 font-medium">Provider ID</th><th className="px-4 py-2 font-medium">Credential</th><th className="px-4 py-2 font-medium">URL</th></tr></thead><tbody className="divide-y divide-boh-border-light dark:divide-boh-border">{resources.map((resource)=>{const connection=connectionById.get(resource.connection_id);return <tr key={resource.id} className="align-top"><td className="px-4 py-2 font-medium">{projectNameById.get(resource.project_id)||'Unknown project'}</td><td className="px-4 py-2"><p className="font-medium">{resource.display_name}</p><p className="text-xs text-boh-text-sub-light dark:text-boh-text-sub">{formatSwitchboardStatus(resource.resource_kind)}</p></td><td className="px-4 py-2">{formatSwitchboardStatus(resource.environment_scope)}</td><td className="max-w-[14rem] break-all px-4 py-2 font-mono text-xs text-boh-text-sub-light dark:text-boh-text-sub">{resource.external_resource_id}</td><td className="px-4 py-2 text-xs text-boh-text-sub-light dark:text-boh-text-sub">{connection?.credential_vault_item_id?'Provider credential linked':'Provider credential not linked'}</td><td className="max-w-[16rem] break-all px-4 py-2 text-xs text-boh-text-sub-light dark:text-boh-text-sub">{resource.service_url||'—'}</td></tr>;})}</tbody></table></div>}</section>;})}<p className="text-xs text-boh-text-sub-light dark:text-boh-text-sub">Provider credential status belongs to the provider connection. It does not mean every linked project service is broken.</p></div>}</>;
}
